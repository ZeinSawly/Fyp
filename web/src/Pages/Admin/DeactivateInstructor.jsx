...
export default function DeactivateInstructor() {
  ...
  const handleSearch = async (values) => {
    ...
    setInstructor(null);
    try {
      const res = await api.get(`/api/admin/lookup-instructor/${values.id.trim()}`);
      setInstructor(res.data.data);
      setStep(1);
    } catch (err) {
      if (err.response?.status === 404) {
        setError(`No instructor found with ID ${values.id}. Please check the ID and try again.`);
      } else {
  ...
  const handleConfirm = () => {
    if (instructor?.status === 'inactive') {
      setError('This instructor account is already deactivated.');
      return;
    }
    setStep(2);
  };
    ...
    Modal.confirm({
      ...
      content: (
        <div>
          ...
          <p style={{ fontWeight: 700, fontSize: 15, color: '#1a365d' }}>{instructor.name}</p>
          <p style={{ color: '#64748B', fontSize: 13 }}>ID: {instructor.id}</p>
          <p style={{ color: '#d97706', fontSize: 13, marginTop: 8 }}>
            The instructor will no longer be able to log in.
          </p>
        </div>
      ),
      ...
      onOk: async () => {
        ...
        setError('');
        try {
          await api.post('/api/admin/deactivate-instructor', {
            id: instructor.id,
            reason: values.reason,
          });
          setSuccess(`Instructor "${instructor.name}" (ID: ${instructor.id}) has been deactivated successfully.`);
          setStep(3);
        } catch (err) {
          setError(err.response?.data?.message || 'Failed to deactivate instructor.');
        } finally {
  ...
  return (
    <AdminLayout title="Deactivate Instructor">
      <div style={{ maxWidth: 700, margin: '0 auto' }}>
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ color: 'rgba(255,255,255,0.75)', fontSize: 13 }}>
              Disable an instructor account without deleting any data
            </div>
          </div>
        </div>
        ...
        <Card style={{ borderRadius: 16, border: 'none', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', marginBottom: 20 }}>
          <Steps
            ...
            size="small"
            items={[
              { title: 'Search Instructor', icon: <SearchOutlined /> },
        ...
        {step === 0 && (
          <Card style={{ borderRadius: 16, border: 'none', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
            <Title level={5} style={{ color: '#1a365d', marginBottom: 6 }}>
              Step 1 — Search for Instructor
            </Title>
            <Text style={{ color: '#64748B', fontSize: 13, display: 'block', marginBottom: 20 }}>
              Enter the instructor ID to look up their account before deactivating.
            </Text>
        ...
        {step === 1 && instructor && (
          <Card style={{ borderRadius: 16, border: 'none', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
              <div>
                <Title level={5} style={{ color: '#1a365d', margin: 0 }}>
                  Step 2 — Verify Instructor Details
                </Title>
                <Text style={{ color: '#64748B', fontSize: 13 }}>
                  Make sure this is the correct instructor before proceeding.
                </Text>
              </div>
              <Tag
                color={instructor.status === 'active' ? 'green' : 'red'}
                style={{ borderRadius: 20, fontWeight: 700, fontSize: 12 }}
              >
                {instructor.status === 'active' ? 'Active' : 'Already Inactive'}
              </Tag>
            </div>
            ...
            <div style={{ background: '#F8FAFC', borderRadius: 12, padding: 20, marginBottom: 20 }}>
              <Row gutter={[16, 16]}>
                {[
                  { label: 'Full Name', value: instructor.name, icon: <UserOutlined />, color: '#2b6cb0' },
                  { label: 'Instructor ID', value: instructor.id, icon: <IdcardOutlined />, color: '#2b6cb0' },
                  { label: 'Email', value: instructor.email || '—', icon: <MailOutlined />, color: '#2f855a' },
                  { label: 'Phone', value: instructor.phone || '—', icon: <PhoneOutlined />, color: '#2f855a' },
                  { label: 'Department', value: instructor.department_name || '—', icon: <ApartmentOutlined />, color: '#d97706' },
                  { label: 'Date of Birth', value: formatDate(instructor.dob), icon: <CalendarOutlined />, color: '#6b46c1' },
                ].map((item, i) => (
        ...
        {step === 2 && instructor && (
          <Card style={{ borderRadius: 16, border: 'none', boxShadow: '0 2px 12px rgba(0,0,0,0.06)' }}>
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontWeight: 700, color: '#1E293B', fontSize: 14 }}>{instructor.name}</div>
                <div style={{ color: '#64748B', fontSize: 12 }}>
                  ID: {instructor.id} · {instructor.department_name}
                </div>
              </div>
            </div>
            ...
            <Form form={reasonForm} layout="vertical" onFinish={handleDeactivate} requiredMark={false}>
              ...
              >
                <TextArea
                  placeholder="e.g. Contract ended, resignation, disciplinary action..."
                  rows={4}
        ...
        {step === 3 && (
          <Card style={{ borderRadius: 16, border: 'none', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', textAlign: 'center', padding: '20px 0' }}>
            ...
            <Text style={{ color: '#64748B', fontSize: 14, display: 'block', marginBottom: 24 }}>
              The instructor account has been successfully deactivated.
            </Text>
